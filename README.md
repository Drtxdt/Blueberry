...
Windows + PowerShell 7 的原生终端补全宿主，当前为 **0.1 Alpha**。独立 Rust 工程，不依赖 Node、npm 或 TypeScript 运行时。PowerShell 适配脚本嵌入可执行文件，负责读取 PSReadLine 输入和应用经过校验的替换。

当前实测热态菜单 P50 为 25.2 ms，适配脚本带来的首次输入增量约 119 ms，**启动目标仍待优化**。详细方法、原始数据与剩余工作见 [性能验收](docs/performance.md)。
...
shellsense probe --iterations 10 --output benchmark.json
shellsense probe --host --iterations 10 --output host-benchmark.json
...
`probe --host` 测量完整宿主的首个提示符、首次菜单、热态菜单和自身 working set。它使用固定配置和临时命令目录，不加载 profile；内存统计排除 pwsh。测试用两层 ConPTY模拟运行环境，OS 缓存不清空。
