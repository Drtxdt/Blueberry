...
use crossterm::{cursor::MoveTo,event::{self,Event,KeyCode,KeyEventKind,KeyModifiers,MouseEventKind},execute,terminal::{Clear,ClearType}};
use serde::{Deserialize, Serialize};
use std::{collections::{BTreeMap,HashSet}, env, fs, io::{self, IsTerminal, Write}, path::{Path, PathBuf},sync::OnceLock};
...
static COMMANDS_CACHE:OnceLock<Commands>=OnceLock::new();
fn cached_commands()->Commands{COMMANDS_CACHE.get_or_init(||fs::read_to_string(config::commands_path()).ok().and_then(|s|toml::from_str(&s).ok()).unwrap_or_default()).clone()}
...
pub fn merged_history(limit:usize,session:&[String],exact_path:Option<&Path>)->Vec<String>{
    let files=exact_path.map(|p|vec![p.to_path_buf()]).unwrap_or_default();
...
fn history(limit:usize)->Vec<SavedCommand>{
    let mut values=history_files().iter().flat_map(|path|read_history_file(path)).collect::<Vec<_>>();let mut seen=HashSet::new();values.reverse();values.into_iter().filter(|line|!line.trim().is_empty()&&seen.insert(line.clone())).take(limit).map(|command|SavedCommand{name:command.lines().next().unwrap_or("").to_owned(),command,description:"PSReadLine 历史".into(),tags:Vec::new()}).collect()
}
...
pub fn run(config_path:Option<&Path>, query:Option<&str>)->Result<u32>{
    ...
    if !io::stdin().is_terminal()||!io::stdout().is_terminal(){let words=query.to_lowercase();for item in items.iter().filter(|i|hub_match(i,&words)).take(100){println!("{}\t{}",item.command,item.name);}return Ok(0)}
    let screen=crate::terminal_ui::ScreenGuard::enter(true)?;let mut selected=0usize;let mut preview=false;let command=loop{let visible=items.iter().enumerate().filter(|(_,item)|hub_match(item,&query.to_lowercase())).map(|(index,_)|index).collect::<Vec<_>>();selected=selected.min(visible.len().saturating_sub(1));draw_hub(&items,&visible,selected,&query,preview)?;match event::read()?{Event::Key(key)if key.kind!=KeyEventKind::Release=>match key.code{KeyCode::Esc=>break None,KeyCode::Char('c')if key.modifiers.contains(KeyModifiers::CONTROL)=>break None,KeyCode::Char(ch)if !key.modifiers.intersects(KeyModifiers::CONTROL|KeyModifiers::ALT)=>{query.push(ch);selected=0;preview=false},KeyCode::Backspace=>{query.pop();selected=0;preview=false},KeyCode::Up=>selected=selected.checked_sub(1).unwrap_or(visible.len().saturating_sub(1)),KeyCode::Down=>selected=if visible.is_empty(){0}else{(selected+1)%visible.len()},KeyCode::PageUp=>selected=selected.saturating_sub(10),KeyCode::PageDown=>selected=(selected+10).min(visible.len().saturating_sub(1)),KeyCode::Tab|KeyCode::F(1)=>preview=!preview,KeyCode::Enter=>{if let Some(index)=visible.get(selected){if let Some(command)=fill_interactive(&items[*index].command)?{break Some(command)}}},_=>{}},Event::Paste(text)=>{query.push_str(&text.replace(['\r','\n']," "));selected=0},Event::Mouse(mouse)=>match mouse.kind{MouseEventKind::ScrollUp=>selected=selected.saturating_sub(3),MouseEventKind::ScrollDown=>selected=(selected+3).min(visible.len().saturating_sub(1)),_=>continue},_=>{}}};
    drop(screen);if let Some(command)=command{copy_or_print(&command)?}
...
fn hub_match(item:&SavedCommand,query:&str)->bool{let text=format!("{} {} {} {}",item.name,item.command,item.description,item.tags.join(" ")).to_lowercase();query.split_whitespace().all(|word|text.contains(word))}
fn draw_hub(items:&[SavedCommand],visible:&[usize],selected:usize,query:&str,preview:bool)->Result<()>{let(cols,rows)=crossterm::terminal::size().unwrap_or((100,30));let mut out=io::stdout();write!(out,"\x1b[?2026h")?;execute!(out,MoveTo(0,0),Clear(ClearType::All))?;writeln!(out,"Blueberry 命令工作台")?;writeln!(out,"搜索：{}",if query.is_empty(){"输入名称、命令、说明或标签"}else{query})?;writeln!(out,"{}","─".repeat(cols as usize))?;let count=rows.saturating_sub(7)as usize;let start=selected.saturating_sub(count.saturating_sub(1));for(row,index)in visible.iter().skip(start).take(count).enumerate(){let item=&items[*index];writeln!(out,"{}",crate::terminal_ui::fit(&format!("{} {:<24}  {}",if start+row==selected{"▶"}else{" "},item.name,item.command),cols as usize))?}if preview{if let Some(index)=visible.get(selected){let item=&items[*index];execute!(out,MoveTo(0,rows.saturating_sub(4)))?;write!(out,"预览：{}\n{}",crate::terminal_ui::fit(&item.command.replace('\n'," ↵ "),cols as usize),crate::terminal_ui::fit(&item.description,cols as usize))?}}execute!(out,MoveTo(0,rows.saturating_sub(1)))?;write!(out,"↑↓ 选择 · Enter 填写/复制 · Tab/F1 预览 · Esc 返回")?;write!(out,"\x1b[?2026l")?;out.flush()?;Ok(())}
fn fill_interactive(command:&str)->Result<Option<String>>{let fields=placeholder_names(command);if fields.is_empty(){return Ok(Some(command.into()))}let mut values=BTreeMap::new();for(index,name)in fields.iter().enumerate(){let mut value=String::new();loop{let(cols,rows)=crossterm::terminal::size().unwrap_or((100,30));let mut out=io::stdout();write!(out,"\x1b[?2026h")?;execute!(out,MoveTo(0,0),Clear(ClearType::All))?;writeln!(out,"填写模板参数  {}/{}",index+1,fields.len())?;writeln!(out,"\n{name}：{value}")?;writeln!(out,"\n预览：{}",crate::terminal_ui::fit(&fill_placeholders(command,&values),cols as usize))?;execute!(out,MoveTo(0,rows.saturating_sub(1)))?;write!(out,"输入参数 · Enter 下一项 · Esc 取消")?;write!(out,"\x1b[?2026l")?;out.flush()?;match event::read()?{Event::Key(key)if key.kind!=KeyEventKind::Release=>match key.code{KeyCode::Esc=>return Ok(None),KeyCode::Enter if !value.trim().is_empty()=>break,KeyCode::Char(ch)if !key.modifiers.intersects(KeyModifiers::CONTROL|KeyModifiers::ALT)=>value.push(ch),KeyCode::Backspace=>{value.pop();},_=>{}},Event::Paste(text)=>value.push_str(&text.replace(['\r','\n']," ")),Event::Mouse(_)=>continue,_=>{}}}values.insert(name.clone(),value);}Ok(Some(fill_placeholders(command,&values)))}
fn copy_or_print(command:&str)->Result<()>{#[cfg(windows)]{use std::process::{Command,Stdio};if let Ok(mut child)=Command::new("clip.exe").stdin(Stdio::piped()).spawn(){if let Some(mut input)=child.stdin.take(){input.write_all(command.as_bytes())?}if child.wait().is_ok_and(|status|status.success()){println!("已复制命令：{command}");return Ok(())}}}println!("{command}");Ok(())}

pub fn candidates(settings:&config::Config, query:&str)->Vec<Candidate>{
    let history=history(settings.workbench.history_limit).into_iter().map(|item|item.command).collect::<Vec<_>>();candidates_with_history(settings,query,&history,None)
}
...
pub fn suggestions(settings:&config::Config,line:&str,session:&[String])->Vec<Candidate>{
    ...
    let needle=line.to_lowercase();let saved=cached_commands();
    let mut values=saved.favorites.into_iter().map(|item|(item.command,item.description,"收藏".to_owned())).collect::<Vec<_>>();
    let scan_limit=settings.workbench.suggestion_limit.saturating_mul(20).max(50).min(settings.workbench.history_limit);
    values.extend(session.iter().take(scan_limit).cloned().map(|command|(command,"最近使用".into(),"历史".into())));
