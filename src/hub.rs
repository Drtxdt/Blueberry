...
fn cached_commands() -> Commands {
    COMMANDS_CACHE
        .get_or_init(|| {
            fs::read_to_string(config::commands_path())
                .ok()
...
fn save_commands(path: &Path, commands: &Commands) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
    fs::write(path, toml::to_string_pretty(commands)?)
        .with_context(|| format!("无法写入 {}", path.display()))
}

pub fn add_favorite(name: &str, command: &str) -> Result<()> {
    let path = config::commands_path();
    let mut saved: Commands = fs::read_to_string(&path)
        .ok()
        .and_then(|s| toml::from_str(&s).ok())
        .unwrap_or_default();
    saved.schema_version = 2;
    if let Some(item) = saved.favorites.iter_mut().find(|item| item.name == name) {
        item.command = command.into();
    } else {
        saved.favorites.push(SavedCommand {
            name: name.into(),
            command: command.into(),
            description: "用户收藏".into(),
            tags: Vec::new(),
        });
    }
    save_commands(&path, &saved)?;
    println!("已保存收藏：{name}");
    Ok(())
}
pub fn remove_favorite(name: &str) -> Result<()> {
    ...
    let mut saved: Commands = fs::read_to_string(&path)
        ...
        .and_then(|s| toml::from_str(&s).ok())
        .unwrap_or_default();
    let before = saved.favorites.len();
    saved.favorites.retain(|item| item.name != name);
    save_commands(&path, &saved)?;
    println!("已删除 {} 个同名收藏。", before - saved.favorites.len());
    Ok(())
}
...
pub fn merged_history(limit: usize, session: &[String], exact_path: Option<&Path>) -> Vec<String> {
    ...
    let mut lines = files
        .iter()
        .flat_map(|p| read_history_file(p))
        .collect::<Vec<_>>();
...
pub fn run(config_path: Option<&Path>, query: Option<&str>) -> Result<u32> {
    ...
    let path = config::commands_path();
    let saved: Commands = fs::read_to_string(&path)
        .ok()
        .and_then(|s| toml::from_str(&s).ok())
        .unwrap_or_default();
            ...
            Event::Key(key) if key.kind != KeyEventKind::Release => match key.code {
                ...
                KeyCode::Enter => {
                    if let Some(index) = visible.get(selected)
                        && let Some(command) = fill_interactive(&items[*index].command)?
                    {
                        break Some(command);
                    }
                }
...
fn draw_hub(
    ...
    write!(out, "↑↓ 选择 · Enter 填写/复制 · Tab/F1 预览 · Esc 返回")?;
    write!(out, "\x1b[?2026l")?;
    out.flush()?;
    Ok(())
}
fn fill_interactive(command: &str) -> Result<Option<String>> {
    let fields = placeholder_names(command);
    if fields.is_empty() {
    ...
    for (index, name) in fields.iter().enumerate() {
        ...
        loop {
            ...
            write!(out, "\x1b[?2026h")?;
            execute!(out, MoveTo(0, 0))?;
            writeln!(out, "填写模板参数  {}/{}", index + 1, fields.len())?;
            writeln!(out, "\n{name}：{value}")?;
            writeln!(
                out,
                "\n预览：{}",
                crate::terminal_ui::fit(&fill_placeholders(command, &values), cols as usize)
            )?;
            execute!(out, MoveTo(0, rows.saturating_sub(1)))?;
    ...
    Ok(Some(fill_placeholders(command, &values)))
}
...
pub fn candidates_with_history(
    ...
    items
        ...
        .map(|(index, item)| Candidate {
            ...
            id: format!("hub:{index}:{}", item.command),
            source: if item.description.contains("历史") {
                "Blueberry 工作台/历史".into()
            } else if item.tags.iter().any(|t| t.contains("操作")) {
