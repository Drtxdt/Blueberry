...
fn stats(values: &[f64]) -> Value {
    ...
    json!({"samples":values,"median":sorted[sorted.len()/2],"p95":sorted[((sorted.len() as f64*0.95).ceil() as usize).saturating_sub(1)],"unit":"ms"})
}
...
pub fn run(shell: &Path, iterations: u16, no_profile: bool) -> Result<Value> {
    ...
    let input_deltas: Vec<f64> = integrated_input
        ...
        .collect();
    Ok(json!({
        "schema":1,"build":if cfg!(debug_assertions) {"debug"} else {"release"},
